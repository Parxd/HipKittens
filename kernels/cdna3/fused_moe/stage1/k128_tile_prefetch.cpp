...
void kernel(const moe_stage1_globals g) {
    ...
    // pure-arithmetic (+ one sorted_expert_ids load) tile-coordinate mapping; safe to run ahead
    auto compute_tile_coords = [&](int t, int& om, int& on, int& ex) {
        const int rb = base_bidx + t * gridDim.x;
        const int num_wgid_in_group = window_size * num_n_tiles;
        const int group_id = rb / num_wgid_in_group;
        const int first_pid_m = group_id * window_size;
        const int group_size_m = min(num_valid_m_tiles - first_pid_m, window_size);
        om = first_pid_m + ((rb % num_wgid_in_group) % group_size_m);
        on = (rb % num_wgid_in_group) / group_size_m;
        ex = g.sorted_expert_ids[om];
    };
    ...
    // tile-0 (k=0) staging buffers, kept live across the tile loop so tile i+1's first global
    ...
    for (int tile = 0; tile < num_tiles_per_cu && base_bidx + tile * gridDim.x < total_tiles; ++tile) {
        ...
        // land tile-0 data prefetched by the prologue (tile 0) or the previous epilogue (tile > 0)
        store_register_buffer_to_shared<NUM_THREADS>(As, a_tile0_buf);
        store_register_buffer_to_shared<NUM_THREADS>(Bs, b_tile0_buf);
        ...
        // Kick off the NEXT output tile's k=0 global loads now, so their latency is hidden behind
        // this tile's epilogue (final MMA + dequant + silu + scatter) instead of a serial prologue.
        const int next_tile = tile + 1;
        const bool have_next = (next_tile < num_tiles_per_cu) && (base_bidx + next_tile * gridDim.x < total_tiles);
