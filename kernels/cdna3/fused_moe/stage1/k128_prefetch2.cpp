...
void kernel(const moe_stage1_globals g) {
    ...
    for (int tile = 0; tile < num_tiles_per_cu && base_bidx + tile * gridDim.x < total_tiles; ++tile) {
        ...
        // 2-stage register prefetch: two buffer sets alternated by K_TILE parity so all
        // buffer references stay compile-time (no dynamically-indexed VGPR arrays / scratch).
        float4 a_buf0[BUFFER_SIZE_A], a_buf1[BUFFER_SIZE_A];
        float4 b_buf0[BUFFER_SIZE_B], b_buf1[BUFFER_SIZE_B];
        ...
        // prime the pipeline: tile 1 lands in buf0 (consumed by iteration 0's store)
        load_global_to_register_buffer<2, false, NUM_THREADS>(b_buf0, BUFFER_SIZE_B, g.B, {0, expert, output_n, 1}, Bs);
        gather_load_global_to_register_buffer<NUM_THREADS>(a_buf0, BUFFER_SIZE_A, g.A, {0, 0, output_m, 1}, g.sorted_token_ids, As);

        for (int K_TILE = 0; K_TILE < k_iters - 1; ++K_TILE) {
            // issue the load for tile K_TILE+2 into the "other" buffer (consumed next iter's store)
            if (K_TILE + 2 < k_iters) {
                if ((K_TILE & 1) == 0) {
                    load_global_to_register_buffer<2, false, NUM_THREADS>(b_buf1, BUFFER_SIZE_B, g.B, {0, expert, output_n, K_TILE + 2}, Bs);
                    gather_load_global_to_register_buffer<NUM_THREADS>(a_buf1, BUFFER_SIZE_A, g.A, {0, 0, output_m, K_TILE + 2}, g.sorted_token_ids, As);
                } else {
                    load_global_to_register_buffer<2, false, NUM_THREADS>(b_buf0, BUFFER_SIZE_B, g.B, {0, expert, output_n, K_TILE + 2}, Bs);
                    gather_load_global_to_register_buffer<NUM_THREADS>(a_buf0, BUFFER_SIZE_A, g.A, {0, 0, output_m, K_TILE + 2}, g.sorted_token_ids, As);
                }
            }

            // read current tile (K_TILE) from LDS + mma
            ...
            // store tile K_TILE+1 (loaded a full iteration ago) into the single LDS buffer
            asm volatile("s_waitcnt lgkmcnt(0)");
            __builtin_amdgcn_s_barrier();  // WAR: current-tile reads done before overwrite
            if ((K_TILE & 1) == 0) {
                store_register_buffer_to_shared<NUM_THREADS>(As, a_buf0);
                store_register_buffer_to_shared<NUM_THREADS>(Bs, b_buf0);
            } else {
                store_register_buffer_to_shared<NUM_THREADS>(As, a_buf1);
                store_register_buffer_to_shared<NUM_THREADS>(Bs, b_buf1);
            }
            asm volatile("s_waitcnt lgkmcnt(0)");
