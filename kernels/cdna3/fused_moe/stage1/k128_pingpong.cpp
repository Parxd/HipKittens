...
void kernel(const moe_stage1_globals g) {
    ...
    for (int tile = 0; tile < num_tiles_per_cu && base_bidx + tile * gridDim.x < total_tiles; ++tile) {
        ...
        // ping-pong offset: stagger the two warp-rows by one cluster
        if (warp_row == 1) {
            __builtin_amdgcn_s_barrier();
        }
        ...
        for (int K_TILE = 0; K_TILE < k_iters - 1; ++K_TILE) {
            ...
            // Cluster 0 (M): prefetch next K-tile + load all current-tile subtiles from LDS.
            // All LDS reads live here so the Cluster 2 store can safely overwrite the buffer.
            ...
            load(b_tiles[0], subtile_inplace<REG_N, REG_K>(Bs, {warp_col, 0}));      // gate, subchunk 0
            load(b_tiles[1], subtile_inplace<REG_N, REG_K>(Bs, {warp_col + 4, 0}));  // up,   subchunk 0
            load(b_tiles[2], subtile_inplace<REG_N, REG_K>(Bs, {warp_col, 1}));      // gate, subchunk 1
            load(b_tiles[3], subtile_inplace<REG_N, REG_K>(Bs, {warp_col + 4, 1}));  // up,   subchunk 1
            __builtin_amdgcn_s_barrier();
            __builtin_amdgcn_sched_barrier(0);

            // Cluster 1 (C): subchunk 0
            ...
            // Cluster 2 (M): store next tile into the single LDS buffer
            asm volatile("s_waitcnt lgkmcnt(0)");  // WAR: current-tile reads retired
            store_register_buffer_to_shared<NUM_THREADS>(As, a_buffer_next);
            store_register_buffer_to_shared<NUM_THREADS>(Bs, b_buffer_next);
            asm volatile("s_waitcnt lgkmcnt(0)");  // RAW: stores visible before next-iter reads
            __builtin_amdgcn_s_barrier();
            __builtin_amdgcn_sched_barrier(0);

            // Cluster 3 (C): subchunk 1 (operands already in registers from Cluster 0)
            __builtin_amdgcn_s_setprio(1);
        ...
        // epilogue: final K-tile already resident in LDS
        __builtin_amdgcn_sched_barrier(0);
        ...
        // ping-pong close: mirror the prologue offset so barrier counts stay balanced
        if (warp_row == 0) {
            __builtin_amdgcn_s_barrier();
        }
